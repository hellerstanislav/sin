...
\begin{itemize}
  ...
  \item umělí mravenci nejsou zcela slepí -- můžou se rozhodovat na základě dalších kritérií, %, např. výběr kratší cesty
...
%Each of artificial ants builds a solution by walking from vertex to vertex on the graph with the constraint of not visiting any vertex that he has already visited in his walk. At each step of the solution construction, an ant selects the following vertex to be visited according to a stochastic mechanism that is biased by the pheromone: when in vertex i, the following vertex is selected stochastically among the previously unvisited ones. In particular, if j has not been previously visited, it can be selected with a probability that is proportional to the pheromone associated with edge (i, j).


%Při řešení problému obchodního cestujícího máme množinu měst a známe vzdálenosti mezi každým z nich. Cílem
%je najít nejkratší cestu mezi těmito městy při navštívení každého města pouze jednou. Formálně, cílem
%je najít Hamiltonovskou kružnici minimální délky v plně propojeném grafu. V rámci ACO je problém řešen
%simulací určitého poštu umělých mravenců, kteří se pohybují grafem. Každý vrchol grafu reprezentuje město,
%každá hrana propojení mezi dvěma městy. Proměnná feromonu je asociována s každou hranou grafu a může být mravenci
%čtena a upravována.

%AS předpokládá, že řešení je rozdělitelné do jednotlivých komponent, kterých je konečný a rozumně omezený počet.
%Zde jsou komponentami hrany grafu. Účelová funkce je součet vah hran na kružnici. Úlohou každého mravence
%je konstrukce platného řešení, tedy platné kružnice v grafu.
...
%První algoritmus, který byl v rámci ACO uveden, byl {\it Ant System} (AS), a to zejména pro řešení problému
%obchodního cestujícího (TSP). Později byla optimalizace mravenčí kolonií formalizována jako celá metaheuristika
%pro řešení různých kombinatorických problémů. Obecně může být použita pro řešení jakéhokoli optimalizačního
%problému, pro který lze použít nějakou konstruktivní heuristickou funkci.

%Mravenci fungují jako stochastické konstruktivní procedury, které vytvářejí řešení iterativním přidáváním
%komponent do částečného řešení, přičemž při výběru každé další komponenty uvažují heuristickou informaci
%o řešeném problému, která směřuje výpočet k slibným řešením, a také zkušenosti získané všemi mravenci od
%začátku výpočtu, reprezentované feromonovými stopami, které se během výpočtu neustále adaptují.

...
feromonové stopy, které pak mají tendenci s větší pravděpodobností následovat. Roku 1990 zkoumal
...
odděleni dvěma mosty. Obrázek \ref{alg:aco} popisuje chování mravenců zjištěné v experimentu při rozdílné
...
Většinu metod inteligence roje lze zařadit mezi tzv. {\it metaheuristiky}, což je způsob optimalizace,
kdy je snahou iterativně zlepšovat nějaké kandidátní řešení s ohledem na požadovanou kvalitu. Těmto
...
optimalizace. Metaheuristika obvykle označuje celou skupinu algoritmických konceptů,
které definují heuristické metody aplikovatelné na širokou škálu různých problémů\footnote{Jinými slovy,
metaheuristika je obecný algoritmický framework, který může být aplikovatelný na mnoho různých
optimalizačních problémů s relativně malými modifikacemi.}.  Mezi tyto metody patří například i
evoluční algoritmy \cite{Dorigo06antcolony}.
%TODO jeste neco o spojite?
...
Hlavním rysem je hledání nejkratší cesty za pomocí nepřímé výměny informací přes okolí (stigmergie, viz def. \ref{def:stig}) pomocí feromonů.

\subsubsection{Princip}
Pro implementaci algoritmů ASO je prvním krokem nejprve diskretizovat a zjednodušit model chování reálných
mravenců, který byl uveden v kapitole \ref{sub:biolzakl} a zejména na obrázku \ref{fig:aco}.
...
\begin{algorithm}[here]
  ...
  \While{\textnormal{termination condition not met}}{
    {\it ConstructAntSolutions}\;
    {\it ApplyLocalSearch} (optional)\;
    {\it UpdatePheromones}\;
  }  
\caption{The Ant Colony Optimization Metaheuristic\label{alg:aco}}
\end{algorithm}
...
Do kategorie ACO, jakožto metaheuristiky, spadá mnoho různých algoritmů, které se většinou liší hlavně způsobem aktualizace feromonové stopy. Zde je probrán nejprve původní algoritmus a poté dvě další, nejúspěšnější varianty.
Obsáhlejší seznam dalších algoritmů lze nalézt například v \cite{Dorigo06antcolony}.
...
Z dalších klasických NP-těžkých optimalizačních problému můžeme jmenovat vytváření rozvrhů,
...
z bioinformatiky.
Výzkum se dále zaměřuje na použití ACO ve víceúčelové, dynamické a stochastické optimalizaci.
...
%TODO Srovnání s genetickými algoritmy? (fitness? - i u PSO?)
...
%\item[(Vybrat ještě něco?] \url{http://en.wikipedia.org/wiki/Swarm_intelligence})

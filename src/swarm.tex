...
požado-vaných geometrických obrazců a vzorů pomocí hejna robotů bez centralizované koordinace.
...
\begin{itemize}
  ...
  \item {\bf Skládání} {\it (Self-assembly)} je úkaz pozorovaný u mravenců, kteří vytváření
    řetězcovité formace, pomocí kterých vytváří "mosty", aby zůstali nad vodou. Problém skládání
...
shnout pod pojem "inženýrství samoorganizace" ({\it engineering of self-organization}).
...
hlavní směry \cite{Blum08SwarmInt}[90-91]:
...
studií {\bf CITE!} byla použita jednoduchá vícevrstvá dopředná nebo rekurentní neuronová síť,
...
V této kapitole se budeme zabývat fyzickou realizací robotického swarmu. Cílem kapitoly
je vytvoření přehledu existujících technologií, které jsou využívány pro implementaci
robotů, které pak tvoří swarm. Nebudeme se tedy zabývat realizací jednotlivých robotů,
jejich motory, senzory atp., ale prvky, které jsou z hlediska robotiky speciální a které
se využívají pouze v multirobotických systémech. Jako příklady zde uvedeme existující
fyzické systémy včetně porovnání funkcionality a použitých technologií.

TODO.
\paragraph{Komunikace.} Pro komunikaci mezi roboty se dnes používají moduly založené na
IR, dále IEEE 802.15.4/ZigBee (roboti e-Puck a Kobot) a často také IEEE 802.11/Wi-Fi (s-bot, Centibot).

\paragraph{Fyzická interakce.} Blabla.
